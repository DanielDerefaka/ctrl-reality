export default function generate(THREE) {
 ...
 loft(lower,[[-.065,.18,.096],[.02,.145,.083],[.10,.138,.080],[.19,.181,.105],[.29,.206,.119],[.36,.157,.076]],obsidian,32);
 for(const sign of [-1,1]){sweep(lower,[[sign*.045,-.035,.103],[sign*.104,.12,.09],[sign*.151,.26,.115],[sign*.173,.33,.068]],.0028);sweep(lower,[[sign*.08,.025,-.079],[sign*.11,.15,-.09],[sign*.177,.28,-.086]],.003);sweep(lower,[[sign*.12,.07,.059],[sign*.11,.04,.079],[sign*.125,-.02,.082]],.0018);}
 plate(lower,[[-.027,-.075],[.027,-.075],[.020,.28],[0,.33],[-.020,.28]],.009,obsidian,.092);
 ...
 for(const [side,sign]of [['left',-1],['right',1]]){
  ...
  loft(arm,[[.025,.041,.046],[0,.067,.066],[-.045,.076,.064],[-.13,.052,.048],[-.24,.039,.037],[-.27,.043,.039]],lining,22);
 const shoulderPanel=joint(side==='left'?'shoulderPanel':'rightShoulderPanel',shoulder);loft(shoulderPanel,[[.055,.014,.022],[.025,.065,.063],[-.008,.084,.061],[-.06,.065,.050],[-.095,.03,.027]],obsidian,24);sweep(shoulderPanel,Array.from({length:13},(_,i)=>{const a=i/12*Math.PI;return[Math.cos(a)*.084,-.015,Math.sin(a)*.073];}),.003);
 ...
 coatPanel('coatLeft',pelvis,-2.95,-.25,.135,.78);coatPanel('coatRight',pelvis,.25,2.95,.135,.78);coatPanel('coatBack',pelvis,2.90,3.38,.16,.81);
 ...
 root.userData.joints=joints;root.userData.parts=joints;root.userData.height=1.8;root.userData.construction='Recovery: Lofted fitted garment, continuous curved panel skirt, sculpted arm sleeves';
