export default function generate(THREE) {
 const root=new THREE.Group();root.name='maraRoot';const joints={};
 ...
 const obsidian=material('metal',0x12171d,.45,.39),lining=material('fabric',0x131820,.1,.72),bronze=material('metal',0xbda06e,.72,.38),ivory=material('stone',0xd6ccba,.08,.42),cyan=material('interface',0x56e8f2,.35,.25,0x36cddd),recess=material('fabric',0x070b0f,.15,.6);
 ...
 loft(lower,[[-.055,.183,.103],[.035,.14,.088],[.14,.137,.083],[.22,.189,.110],[.31,.213,.118],[.36,.14,.074]],lining,24);
 for(const sign of [-1,1]){for(let i=0;i<3;i++){const y=.04+i*.087;const part=plate(lower,[[sign*.012,y-.04],[sign*(.115+i*.021),y],[sign*(.14+i*.025),y+.089],[sign*.016,y+.05]],.021,obsidian,.085+i*.005);sweep(lower,[[sign*.012,y-.04,.115],[sign*(.115+i*.021),y,.122],[sign*(.14+i*.025),y+.089,.125]],.0025);}sweep(lower,[[sign*.055,-.04,-.088],[sign*.10,.14,-.092],[sign*.18,.31,-.088]],.0026);}
 ...
 for(let j=0;j<=rows;j++){const t=j/rows,y=-.119+t*.266;const silhouette=Math.sin(Math.PI*t)**.53;const width=(.078+.036*t)*silhouette;for(let i=0;i<=cols;i++){const u=i/cols*2-1;const z=.025+.093*Math.sqrt(Math.max(0,1-u*u))*(.82+.18*Math.sin(t*Math.PI))+.020*(1-t);maskPos.push(width*u,y,z);}}
 ...
 for(const [side,sign]of [['left',-1],['right',1]]){
  const shoulder=joint(side+'Shoulder',upper,sign*.203,.199,0),arm=joint(side+'UpperArm',shoulder);
  loft(arm,[[.012,.036,.036],[-.02,.068,.056],[-.095,.055,.043],[-.19,.043,.035],[-.27,.038,.033]],lining,20);const pauldron=joint(side==='left'?'shoulderPanel':'rightShoulderPanel',shoulder);for(let i=0;i<3;i++){const m=plate(pauldron,[[-.055,.018-i*.035],[0,.047-i*.035],[.06,.015-i*.035],[.044,-.036-i*.035],[0,-.051-i*.035],[-.049,-.029-i*.035]],.06,obsidian,-.035);m.rotation.y=sign*.13;}
 ...
 const left=joint('coatLeft',pelvis),right=joint('coatRight',pelvis),back=joint('coatBack',pelvis);for(let i=0;i<6;i++){const a=.30+i*.93;coatPanel('tailPanel'+i,i<2?right:i>3?left:back,a,a+.87,.17,.79+(i%2)*.05);}
 // The seam system is anatomical: shoulder blade, waist chevrons, glove rails.
 for(const sign of [-1,1]){
 ...
 root.userData.joints=joints;root.userData.parts=joints;root.userData.height=1.8;root.userData.construction='Recovery: Anatomical segmented cuirass with six independent overlapping meridian coat panels';
