export default function generate(THREE) {
 ...
 const head=joint('head',neck,0,.105,0);loft(head,[[-.105,.045,.052],[-.05,.093,.09],[.025,.104,.096],[.09,.089,.088],[.139,.042,.055],[.149,.002,.002]],ivory,32);
 ...
 for(const [side,sign]of [['left',-1],['right',1]]){
  const shoulder=joint(side+'Shoulder',upper,sign*.195,.199,0),arm=joint(side+'UpperArm',shoulder);
  loft(arm,[[.025,.041,.046],[0,.067,.066],[-.045,.076,.064],[-.13,.052,.048],[-.24,.039,.037],[-.27,.043,.039]],lining,22);
 const shoulderPanel=joint(side==='left'?'shoulderPanel':'rightShoulderPanel',shoulder);loft(shoulderPanel,[[.035,.026,.028],[.020,.062,.059],[-.015,.072,.063],[-.070,.055,.046],[-.10,.037,.029]],obsidian,24);sweep(shoulderPanel,Array.from({length:13},(_,i)=>{const a=i/12*Math.PI;return[Math.cos(a)*.084,-.015,Math.sin(a)*.073];}),.003);
  ...
  if(side==='left'){sweep(forearm,[[-.059,-.04,-.012],[-.05,-.12,-.018],[-.034,-.23,-.008]],.004,cyan);const core=joint('gloveCore',forearm,0,-.125,.062);plate(core,[[-.007,.085],[.007,.085],[.005,-.08],[-.005,-.08]],.008,cyan);const glow=joint('gloveLight',hand);sweep(glow,[[0,-.009,.025],[0,-.065,.029]],.004,cyan);const ring=joint('gloveRing',forearm,0,-.242,0);const tor=add(new THREE.TorusGeometry(.038,.004,5,24),cyan,ring);tor.rotation.x=Math.PI/2;}
 ...
 coatPanel('coatLeft',pelvis,-2.95,-.25,.10,.70);coatPanel('coatRight',pelvis,.25,2.95,.10,.70);coatPanel('coatBack',pelvis,2.90,3.38,.115,.74);
 root.userData.joints=joints;root.userData.parts=joints;root.userData.height=1.8;root.userData.construction='Lofted fitted garment, continuous curved panel skirt, sculpted arm sleeves';
